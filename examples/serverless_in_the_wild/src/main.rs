...
fn gen_sample<T: Copy>(perc: &Vec<f64>, vals: &Vec<T>) -> T {
...
fn process_azure_trace(path: &Path, invocations_limit: usize) -> Trace {
    ...
    for part in parts.iter() {
        if trace.len() == invocations_limit {
            break;
        }
            ...
            if let Some(inv_vec) = inv_cnt.get(id) {
                ...
                if let Some(mem_vec) = mem_dist.get(&id0) {
                    if !fn_data.contains_key(id) {
                        let idx = fn_data.len();
                        let mem = gen_sample(&mem_percent, mem_vec);
                        fn_data.insert(id.clone(), (idx, 10.0, mem as u64));
                    }
                    let idx = fn_data.get(id).unwrap().0;
                    for i in 0..1440 {
                        for inv in 0..inv_vec[i] {
                            let second = rand::random::<f64>() * 60.0 + ((i * 60 + day * 1440 * 64) as f64);
                            let record = TraceRecord {
                                id: idx,
                                time: second,
                                dur: gen_sample(&dur_percent, dur_vec),
                            };
                            trace.push(record);
                            if trace.len() == invocations_limit {
                                break;
...
fn main() {
    ...
    describe(test_policy(None, &trace));
}
