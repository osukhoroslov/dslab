    ...
    fn on(&mut self, event: Box<dyn Event>, from: &ActorId, ctx: &mut ActorContext) {
        match_event!( event {
            Start {} => {
                ...
                ctx.emit(TaskAssigned { amount: 100 }, &self.worker, 0.);
            },
            TaskCompleted {} => {
...
impl Worker {
    pub fn new(compute: &ActorId) -> Self {
        Self { compute: compute.clone(), master: None }
    }
}
    ...
    fn on(&mut self, event: Box<dyn Event>, from: &ActorId, ctx: &mut ActorContext) {
        match_event!( event {
            TaskAssigned { amount } => {
                ...
                ctx.emit(CompRequest { amount: *amount }, &self.compute, 0.);
            ...
            CompFinished {} => {
                ...
                ctx.emit(TaskCompleted {}, self.master.as_ref().unwrap(), 0.);
            },
            CompFailed {} => {
