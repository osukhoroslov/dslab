...
fn main() {
    ...
    constant_network.borrow_mut().transfer_data_from_sim(sender_actor.clone(), receiver_actor.clone(), 100.0, &mut sim);
    constant_network.borrow_mut().transfer_data_from_sim(sender_actor.clone(), receiver_actor.clone(), 1000.0, &mut sim);
    constant_network.borrow_mut().transfer_data_from_sim(sender_actor.clone(), receiver_actor.clone(), 5.0, &mut sim);

    constant_network.borrow_mut().send_message_from_sim("Hello World".to_string(), receiver_actor.clone(), &mut sim);
