...
pub trait RoutingAlgorithm {
    ...
    /// Returns a path iterator from node `src` to node `dest`.
    ///
    /// Can be used only after calling [`Self::init`].
    fn get_path_iter<'a>(&'a self, src: NodeId, dest: NodeId, topology: &'a Topology) -> Option<PathIterator<'a>>;
}
...
impl RoutingAlgorithm for ShortestPathFloydWarshall {
    ...
    fn get_path_iter<'a>(&'a self, src: NodeId, dest: NodeId, topology: &'a Topology) -> Option<PathIterator<'a>> {
        if self.parent_path[dest][src] == INVALID_NODE_ID {
            None
        } else {
            Some(PathIterator {
                from: src,
