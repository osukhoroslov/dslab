...
impl TestEventHandler {
    fn register_new_component(&mut self, component_name: String) -> SimulationContext {
        ...
        let new_event_handler = Rc::new(RefCell::new(TestEventHandler{sim: self.sim.clone(), new_component_id: Some(context.id())}));
        self.sim.borrow().add_handler(&component_name, new_event_handler.clone());
        context
    }
}
...
impl EventHandler for TestEventHandler {
    fn on(&mut self, event: Event) {
        cast!(match event.data {
            TestEventData { component_name, create_nested_components } => {
                let context = self.register_new_component(component_name);
                if create_nested_components {
                    context.emit_self_now(TestEventData{component_name: "component1".to_string(), create_nested_components: false});
                }
            }
        })
    }
...
fn run_test_simulation(create_nested_components: bool) {
    ...
    let event_handler = Rc::new(RefCell::new(TestEventHandler{sim: sim.clone(), new_component_id: None }));

    let context = sim.borrow_mut().create_context("test_event_handler");
    sim.borrow_mut().add_handler("test_event_handler", event_handler.clone());

    context.emit_self_now(TestEventData{component_name: "component1".to_string(), create_nested_components});
