...
impl EventPromiseStore {
    ...
    pub fn insert<T: EventData>(
        ...
        if self.promises.contains_key(&key) {
            return Err(format!("Event promise for {:?} already exists", key));
        }
        ...
        if let Some(src) = src {
            if let Some(promises) = self.promises_with_source.get(&key) {
                ...
                if promises.contains_key(&src) {
                    return Err(format!(
                        "Event promise for {:?} with source {} already exists",
                        key, src
                    ));
                }
            }
            ...
            if let Some(promises) = self.promises_with_source.get(&key) {
                // check that promise with such key does not exists yet
