...
type BidirectionalMapping = (
    BTreeMap<OrderedFloat<f64>, HashSet<McEventId>>,
    HashMap<McEventId, OrderedFloat<f64>>,
);
...
struct DependencyResolver {
    proc_timers: HashMap<String, BidirectionalMapping>,
    pending_messages: HashMap<String, BidirectionalMapping>,
    event_to_proc: HashMap<McEventId, String>,
}

impl DependencyResolver {
    fn get_all_available_events(&self, proc: &String) -> HashSet<McEventId> {
        ...
        if let Some((time, timers)) = self.get_min_time_timers(proc) {
            if !self.get_min_time_message(proc).map_or(false, |t| t < time) {
                return HashSet::from_iter(messages_iter.chain(timers.clone().into_iter()));
            }
        }
        HashSet::from_iter(messages_iter)
    }

    pub fn add_timer(&mut self, proc: String, time: f64, event: McEventId) -> (bool, Option<HashSet<McEventId>>) {
        ...
        let prev_time = timer_group.range(..OrderedFloat(time)).next_back();
        ...
        let next_time = timer_group.range((Excluded(OrderedFloat(time)), Unbounded)).next();
        let blocked_events = next_time.map(|e| e.1).cloned();

        (is_available, blocked_events)
    }

    pub fn add_message(&mut self, proc: String, time: f64, event: McEventId) -> HashSet<McEventId> {
        self.add_event_to_node_mapping(proc.clone(), event);
    ...
    pub fn cancel_timer(&mut self, proc: String, event: McEventId) -> HashSet<McEventId> {
    ...
    fn create_event_mappings(
        proc_mapping: &mut HashMap<String, BidirectionalMapping>,
        proc: &String,
        id: McEventId,
        time: f64,
    ) {
        let mapping = proc_mapping.entry(proc.clone()).or_default();
        mapping.0.entry(OrderedFloat(time)).or_default().insert(id);
        mapping.1.insert(id, OrderedFloat(time));
    }

    fn get_min_time_message(&self, proc: &String) -> Option<&OrderedFloat<f64>> {
    ...
    fn get_min_time_timers(&self, proc: &String) -> Option<(&OrderedFloat<f64>, &HashSet<McEventId>)> {
    ...
    pub fn pop(&mut self, event: McEventId) -> HashSet<McEventId> {
...
pub struct PendingEvents {
    ...
    available_events: HashSet<McEventId>,
    resolver: DependencyResolver,
    id_counter: usize,
    global_time: BTreeMap<String, f64>,
}
...
impl PendingEvents {
    ...
    pub fn available_events(&self) -> &HashSet<McEventId> {
        ...
        for node_id in 0..3 {
            ...
            for event_time in times {
                let event = McEvent::TimerFired {
                    proc: node_id.to_string(),
                    timer: format!("{}", event_time),
                    duration: 1.0 + event_time as f64,
                };
                rev_id[resolver.push(event)] = event_time * 3 + node_id;
            }
        }
    ...
    fn test_timer_dependency_resolver_stable_network() {
        ...
        for message_time in message_times {
            let event = McEvent::MessageReceived {
                ...
                src: "0".to_owned(),
                dest: "0".to_owned(),
                options: DeliveryOptions::NoFailures(message_time as f64),
            };
        ...
        let count_timers_available =
            |available: &HashSet<McEventId>| available.iter().filter(|x| rev_id[**x] < 20).count();
        let count_messages_available =
            |available: &HashSet<McEventId>| available.len() - count_timers_available(available);
    ...
    fn test_timer_dependency_resolver_message_blocks_timer() {
        ...
        for timer in 0..20 {
            let event = McEvent::TimerFired {
                proc: "0".to_owned(),
                timer: format!("{}", timer),
                duration: 10.0 * (1.0 + (timer / 10) as f64),
            };
        ...
        let message = McEvent::MessageReceived {
            ...
            src: "0".to_owned(),
            dest: "0".to_owned(),
            options: DeliveryOptions::NoFailures(1.0),
        };
