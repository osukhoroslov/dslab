...
fn check_placements<T: SingleVMPlacementAlgorithm + 'static>(algorithm: T, expected_hosts: Vec<&str>) {
    ...
    let scheduler_id = cloud_sim.add_scheduler("s", VMPlacementAlgorithm::single(algorithm));
...
fn test_first_fit() {
    check_placements(FirstFit::new(), vec!["h1", "h2", "h1"]);
}

#[test]
// Tests BestFit algorithm.
fn test_best_fit() {
    check_placements(BestFit::new(), vec!["h1", "h2", "h2"]);
}

#[test]
// Tests WorstFit algorithm.
fn test_worst_fit() {
    check_placements(WorstFit::new(), vec!["h3", "h1", "h2"]);
}

#[test]
// Tests Dot Product algorithm.
fn test_dot_product() {
    check_placements(DotProduct::new(), vec!["h3", "h4", "h1"]);
}

#[test]
// Tests Weighted Dot Product algorithm.
fn test_weighted_dot_product() {
    check_placements(WeightedDotProduct::new(), vec!["h3", "h1", "h4"]);
}
...
fn test_l2_norm_diff() {
    check_placements(L2NormDiff::new(), vec!["h4", "h1", "h4"]);
}
...
fn test_cosine_similarity() {
    check_placements(CosineSimilarity::new(), vec!["h1", "h3", "h3"]);
}
...
fn test_delta_perp_distance() {
    check_placements(DeltaPerpDistance::new(), vec!["h4", "h3", "h3"]);
}
