...
fn test_constant_model() {
    ...
    let state = HostState::new(None, None, Some(1.), Some(1.), None);
...
fn test_ddr3_model() {
    ...
    let mut state = HostState::new(None, None, Some(0.), Some(0.), None);
    assert!(model.get_power(state.clone()) > 23.4);
    assert!(model.get_power(state.clone()) < 23.41);

    state = HostState::new(None, Some(0.5), Some(0.4), Some(0.8), None);
