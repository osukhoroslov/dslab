...
pub trait MemoryPowerModel: DynClone {
    /// Returns memory power consumption in W based on a single utilization value.
    ///
    /// Memory utilization should be passed as a float in 0.0-1.0 range.
    fn get_power_simple(&self, utilization: f64) -> f64;

    /// Returns memory power consumption in W based on separate read and write utilization values.
    ///
