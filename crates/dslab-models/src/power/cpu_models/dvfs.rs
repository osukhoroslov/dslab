...
impl DVFSCpuPowerModel {
    fn validate_model(&self) {
        if self.static_power + (self.util_coef + self.freq_coef) * self.max_power != self.max_power {
            panic!("Incorrect DVFSCpuPowerModel params");
        }
    }
    ...
    /// Creates a linear power model with spesifyed .
    ///
    /// * `static_power` - The minimum power consumption in W (at 0% utilization).
    /// * `max_power` - The maximum power consumption in W (at 100% utilization).
    /// * `util_coef` - The fraction of power generated by flat CPU utilization.
    /// * `freq_coef` - The fraction of power generated by CPU frequency.
    pub fn new_with_coefs(static_power: f64, max_power: f64, util_coef: f64, freq_coef: f64) -> Self {
        Self {
            static_power,
            max_power,
            util_coef,
